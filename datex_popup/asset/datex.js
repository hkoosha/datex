...
(function ($) {
    ...
    function timeSett(settings, hasMinute) {
        return {
            responsive: true,
            persianDigit: false,
            initialValue: false,
            ...
            format: settings.showSeconds ? 'H:m:s' : (hasMinute ? 'H:m' : 'H'),
        };
    }

    function processDate(who, settings, id) {
        ...
        attach: function (ctx) {
            for (var id in Drupal.settings.datePopup) {
                ...
                if (data.func === 'datepicker') {
                    processDate(find, data.settings, id)
                }
                else {
